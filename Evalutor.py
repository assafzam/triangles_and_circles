...
from gbox.gboxes_on_image import GBoxesOnImage, PredGBoxesOnImage
from utils import *


class Evaluator:
    def GetPascalVOCMetrics(self,
                            gt_boxes: GBoxesOnImage,
                            pred_boxes: PredGBoxesOnImage,
                            IOUThreshold=0.5,
                            ):
        ...
        for bb in gt_boxes:
            if bb.class_name not in classes:
                classes.append(bb.class_name)
        ...
        for c in classes:
            ...
            dects = []
            [dects.append(d) for d in pred_boxes if d.class_name == c]
            ...
            for g in groundTruths:
                if g.class_name == c:
                    npos += 1
                    gts[g.class_name] = gts.get(g.class_name, []) + [g]

            # sort detections by decreasing confidence
            dects = sorted(dects, key=lambda box: box.top_prob, reverse=True)
            ...
            for d,d_box in enumerate(dects):
                iouMax = sys.float_info.min
                for j,gt_box in enumerate(gts[c]):
                # for j in range(len(gt)):
                    # print('Ground truth gt => %s' % (gt[j][3],))
                    iou = d_box.iou(gt_box)
                    if iou > iouMax:
                        iouMax = iou
                        jmax = j
                # Assign detection as true positive/don't care/false positive
                if iouMax >= IOUThreshold:
                    if det[d_box.class_name][jmax] == 0:
                        TP[d] = 1  # count as true positive
                        det[d_box.class_name][jmax] = 1  # flag as already 'seen'
                        # print("TP")
                    else:
    ...
    def PlotPrecisionRecallCurve(self,
        ...
        """
        results = self.GetPascalVOCMetrics(gt_boxes, pred_boxes, IOUThreshold)
        result = None
